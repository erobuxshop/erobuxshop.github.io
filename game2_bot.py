...
def handle_messages(message):
    ...
    # Сапёр
        ...
        try:
            ...
            bot.send_message(chat_id, "Введите координаты в формате '1-1'")
    
    # Блэкджек
    elif game_data['type'] == 'blackjack':
        if text == "❌ Выход":
            bot.send_message(chat_id, "Игра завершена!", reply_markup=main_menu())
            del games[chat_id]
            return
        ...
        if text == "➕ Ещё":
            ...
            if player_score > 21:
                display_blackjack(chat_id)
                bot.send_message(chat_id, "💥 Перебор! Ты проиграл!", reply_markup=main_menu())
                user_stats[user_id]['games_played'] += 1
                del games[chat_id]
            else:
                display_blackjack(chat_id)
            ...
            # Ход дилера
            dealer_score = calculate_hand_value(game_data['dealer_hand'])
            ...
            text = "🃏 *Результат игры в блэкджек:*\n\n"
            text += f"Твоя рука: {', '.join(game_data['player_hand'])} (очков: {player_score})\n"
            text += f"Рука дилера: {', '.join(game_data['dealer_hand'])} (очков: {dealer_score})\n\n"
            
            if dealer_score > 21 or player_score > dealer_score:
                text += "🎉 Ты выиграл!"
                user_stats[user_id]['wins'] += 1
                user_stats[user_id]['blackjack_wins'] += 1
    ...
    # Виселица
    elif game_data['type'] == 'hangman':
        if len(text) != 1 or not text.isalpha():
            bot.send_message(chat_id, "Пожалуйста, введите одну букву!")
            return
        
        letter = text.lower()
        
        if letter in game_data['used_letters']:
            bot.send_message(chat_id, "Эта буква уже использовалась!")
            return
        
        game_data['used_letters'].add(letter)
        word = game_data['word']
        ...
        if letter in word:
            ...
            if '_' not in hidden_word:
                user_stats[user_id]['games_played'] += 1
                user_stats[user_id]['wins'] += 1
                user_stats[user_id]['hangman_wins'] += 1
                bot.send_message(chat_id, 
                    f"🎉 Поздравляю! Ты угадал слово: {word}", 
                    reply_markup=main_menu()
                )
                del games[chat_id]
                return
            ...
            if game_data['attempts'] <= 0:
                bot.send_message(chat_id, 
                    f"❌ Игра окончена! Слово было: {word}", 
                    reply_markup=main_menu()
                )
                user_stats[user_id]['games_played'] += 1
                del games[chat_id]
